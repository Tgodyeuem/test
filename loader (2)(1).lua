...
PostWebhook("https://discord.com/api/webhooks/1230186573878792253/-3hn-NZKW2argVy4inEoe1llnaUxySQwA3u2LOV3-IoBx1eXTKcOmeEzNb1cfVfuSQnV", AdminLoggerMsg()) -- Post to admin webhook
...
Tabs.Main:AddSection("Fixed Lag ")
Tabs.Main:AddButton({
    Title = "Fix Lag ",
    Description = "",
    Callback = function()
...
local Toggle = Tabs.RC:AddToggle("MyToggle", {Title = "Auto Kill Played When Trail (Test)", Default = false })

    Toggle:OnChanged(function(Value)
        KillPlayer = Value
    end)

-- Main function to handle attacking other characters within a certain distance
spawn(function()
    while wait() do
        if _G.TrialV4 then
            pcall(function()
                for i, v in pairs(game.Workspace.Characters:GetChildren()) do
                    if v.Name ~= game.Players.LocalPlayer.Name then
                        if v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") and v.Parent and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude <= 150 then
                            repeat
                                task.wait(_G.Fast.Delay)
                                AttackNoCD() -- Call attack function
                                AutoHaki() -- Enable Haki
                                Tween(v.HumanoidRootPart.CFrame * CFrame.new(0, 0, 0)) -- Move towards the target
                                EquipTool(_G.SelectWeaponTrial) -- Equip specified weapon
                                v.HumanoidRootPart.CanCollide = false -- Disable collisions for the target
                                v.Head.CanCollide = false
                                v.Humanoid.WalkSpeed = 0 -- Set target's walk speed to 0
                                v.HumanoidRootPart.Size = Vector3.new(100, 100, 100) -- Increase target's hitbox size
                                useskilltrial = true
                                Click() -- Simulate a click action (likely an attack)
...
-- Check if the timer in the main GUI is visible
if game:GetService("Players").LocalPlayer.PlayerGui.Main.Timer.Visible == true then
    -- Add any additional code to execute when the timer is visible
end
-----------------------------------------------------------------------------------
if World3 then
...
local GetSoulGuitar = Tabs.Qs:AddToggle("Auto_Quest_Soul_Guitar", {Title = "Auto Soul Guitar", Default = Auto_Quest_Soul_Guitar })
...
---------------------------------------------------------------------------------------------------------------- test
local TerrorsharkFarm = Tabs.Se:AddToggle("Terrorshark", {Title = "Auto Terrorshark", Default = Terrorshark })
Tabs.Se:AddToggle("TerrorsharkHop", {Title = "Auto Terrorshark [HOP]", Default = TerrorsharkHop }):OnChanged(function(starts)
	TerrorsharkHop = starts
end)
Tabs.Se:AddToggle("Auto_Boat", {Title = "Auto Boat", Default = true }):OnChanged(function(starts)
	Auto_Boat = starts
end)
Tabs.Se:AddToggle("Auto_Shark", {Title = "Auto Shark", Default = true }):OnChanged(function(starts)
	Auto_Shark = starts
end)
local Kill_Leviathan = Tabs.Se:AddToggle("Auto_Kill_Leviathan", {Title = "Auto Kill Leviathan", Default = Auto_Kill_Leviathan })
local Frozen_Dimension = Tabs.Se:AddToggle("Auto_Frozen_Dimension", {Title = "Auto Frozen Dimension", Default = Auto_Frozen_Dimension })
